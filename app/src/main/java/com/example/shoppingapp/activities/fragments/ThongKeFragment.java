...
import com.example.shoppingapp.adapters.ThongKeAdapter;
...
public class ThongKeFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        FirebaseDatabase.getInstance().getReference("spbc").addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                list=new ArrayList<>();
                for (DataSnapshot dataSnapshot : snapshot.getChildren()){
                    ThongKe thongKe=new ThongKe();
                    thongKe.setSlg(dataSnapshot.child("slg").getValue(Integer.class));
                    Product product=dataSnapshot.child("sp").getValue(Product.class);
                    thongKe.setImg(product.getListImage().get(0));
                    thongKe.setName(product.getName());
                    list.add(thongKe);
                }
                adapter=new ThongKeAdapter(list,getContext());
